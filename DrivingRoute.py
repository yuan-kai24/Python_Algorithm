...
def dfs(end, index, map=[]):
    ...
    map.append(index)
    for item in range(end-1, path.__len__()):
        # 遍历所有元素
        if end == returnkey(path[item]):
            # 查询与上一个路口最后到达的
            map[index] = returnValue(path[item])[1]
            # 添加
            dfs(returnValue(path[item])[0], index+1, map)
            # 搜索
    del map[index]
    ...
    for item in range(0, data.__len__()):
        ...
        for ite in range(0, data[item].__len__()):
            if returnkey(data[item][ite]) == 0:
                ...
                num = 0
                # 统计完小路后置零，因为已经被大路中断
                value = value + returnValue(data[item][ite])
                # 大路疲劳值
            else:
                num = num + returnValue(data[item][ite])
                # 累计小路疲劳值
        value = value + (num*num)
        num = 0
        # 置零操作
